...
void handler_put_key_value() {
    ...
    if (data_read != NULL) {
        ...
        int res = zookeeper_put(entry);

        if (res != -1)
             printf("O identificador da operação PUT é: %d\n",res);
        else
...
void handler_get_key() {
    ...
    struct data_t *data = zookeeper_get(key);
...
void handler_del_key() {
    ...
    int res = zookeeper_del(key);
    if (res != -1)
        printf("O identificador da operação DEL é: %d\n",res);
    else
...
void handler_verify() {
    char *str_op_n = strtok(NULL, SEPARATOR);

    if (str_op_n != NULL) {
        int op_n = atoi(str_op_n);
        int was_executed = zookeeper_verify(op_n);

        if (was_executed > 0)
            printf("Operação número %d já foi executada\n", op_n);
        else if(was_executed == 0)
            printf("Em espera: Operação número %d ainda não foi executada\n", op_n);
        else
            printf("ERRO ao realizar a operação VERIFY\n");
    } else {
        printf("ERRO ao realizar a operação VERIFY (poucos argumentos: verify <op_n>)\n");
    }
}
...
int main(int argc, char **argv)
{
    ...
    while(fgets(buf, MAX, stdin)){
        ...
        else if (strcmp(token, VERIFY_OPN) == 0) { // VERIFY
            handler_verify();
        }
        else {
