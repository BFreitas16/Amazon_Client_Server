...
{   
    ...
    *data_buf = (char *) malloc(total_size);
    if (data_buf == NULL) {
        perror("ERRO no malloc do data_buf do data_to_buffer");
        return -1;
    }
...
struct data_t *buffer_to_data(char *data_buf, int data_buf_size)
{
    ...
    if (net_size == NULL) {
        perror("ERRO no malloc do net_size do buffer_to_data");
        return NULL;
    }
    ...
    if (data == NULL) {
        perror("ERRO no malloc do data do buffer_to_data");
        return NULL;
    }
...
int entry_to_buffer(struct entry_t *data, char **entry_buf)
{
    ...
    *entry_buf = (char *) malloc(total_size);
    if (data_buf == NULL) {
        perror("ERRO no malloc do entry_buf do entry_to_buffer");
        return -1;
    }
...
struct entry_t *buffer_to_entry(char *entry_buf, int entry_buf_size)
{
    ...
    if (net_size_string == NULL) {
        perror("ERRO no malloc do net_size_string do buffer_to_entry");
        return NULL;
    }

    u_int32_t *net_size_data = malloc(sizeof(u_int32_t));
    if (net_size_data == NULL) {
        perror("ERRO no malloc do net_size_data do buffer_to_entry");
        return NULL;
    }
    ...
    char * key = (char *) malloc(size_string);
    if (key == NULL) {
        perror("ERRO no malloc do key do buffer_to_entry");
        return NULL;
    }
    ...
    if (data == NULL) {
        perror("ERRO no malloc do data do buffer_to_entry");
        return NULL;
    }
